...
export async function sendEmail({ to, subject, html }) {
  ...
  try {
    const info = await transporter.sendMail({
      from: `"floocc" <${process.env.EMAIL_FROM}>`,
      to,
      subject,
      html,
    });
...
export function generateVerificationEmail(user, token) {
  ...
  return {
    to: user.email,
    subject: 'Verify your floocc account',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h1 style="color: #4A5568; text-align: center;">Welcome to floocc!</h1>
        <p style="font-size: 16px; color: #4A5568;">Hi ${user.name},</p>
        <p style="font-size: 16px; color: #4A5568;">Thanks for signing up for floocc. To complete your registration, please verify your email address.</p>
        ...
        <p style="font-size: 14px; color: #718096;">If you didn't sign up for floocc, you can safely ignore this email.</p>
